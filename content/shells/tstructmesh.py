...
def MakeTStructureMesh(quads=True, nx1=4, ny1=4, nx2=4, ny2=4, ratio=0.5):
    ...
    for i in range(nx2):
        mesh.Add(meshing.Element1D([pids[int(ny1/2) * (nx1+1) + i], pids[int(ny1/2) * (nx1+1) + i+1]], index=5))#todo!!!
        #print("1:p1 = ", int(ny1/2) * (nx1+1) + i, ", p2 = ", int(ny1/2) * (nx1+1) + i+1)

    mesh.Add(meshing.Element1D([pids[int(ny1/2) * (nx1+1)], pids[base1]], index=6))
    #print("2:p1 = ", int(ny1/2) * (nx1+1), ", p2 = ", base1)
    for i in range(ny2-1):
        mesh.Add(meshing.Element1D([pids[i*(nx2+1)+base1], pids[(i+1)*(nx2+1)+base1]], index=6))
        #print("3:p1 = ", i*(nx2+1)+base1, ", p2 = ", (i+1)*(nx2+1)+base1)
    for i in range(nx2):
        mesh.Add(meshing.Element1D([pids[(ny2-1)*(nx2+1)+i+base1], pids[(ny2-1)*(nx2+1)+i+1+base1]], index=7))
        #print("4:p1 = ", (ny2-1)*(nx2+1)+i+base1, ", p2 = ", (ny2-1)*(nx2+1)+i+1+base1)

    for i in range(ny2-1):
        mesh.Add(meshing.Element1D([pids[(i+1)*(nx2+1)+nx2+base1], pids[i*(nx2+1)+nx2+base1]], index=8))
        #print("5:p1 = ", (i+1)*(nx2+1)+nx2+base1, ", p2 = ", i*(nx2+1)+nx2+base1)

    #print("6:p1 = ", base1+nx1, ", p2 = ", int(ny1/2) * (nx1+1) + nx1)
    mesh.Add(meshing.Element1D([pids[base1+nx1], pids[int(ny1/2) * (nx1+1) + nx1]], index=8))#todo!!!
